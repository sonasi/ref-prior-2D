...
//		cout << "bin " << i << " bin content " << bincontent << " stat error: " << binerror << " eff error: " << bincontent*error << " rel error: " << binerror/bincontent << endl;
	...
	int Mj = 3; // Number of Monte Carlo integration points
...
//	MarginalizedPoissonGamma pgamma(mva_src_b, A2[l], 1.0, 1.0);
 //   vector<double>& xdata = pgamma.generate(2.0); 
	...
	TFile rpfile("output/refprior.root");
	TGraph2D *firstprior = (TGraph2D*) gDirectory->Get("prior");
	Double_t *rpPoints = firstprior->GetZ();

	for(int l=0; l<maxsig-1; l++) {
		vector<double> thisrow;
		for( int j = 0; j < xsec_plots-1; j++)
		{
//			posterior[l][j] = rpPoints[(j+l*xsec_plots)] * likelihood[l][j] ;
//			double point = firstprior->GetBinContent(l,j);
			thisrow.push_back(rpPoints[(j+l*xsec_plots)]);
		}
	...
	MarginalizedPoissonGammaInt pgammaint(B, A2, fB, f2,  1.0, 1.0, 6.0, xsec_plots, maxsig-1, signals, prior);
	cout << "between: " << maxsig << "\t" << xsec_plots << endl;

	JeffreysPrior jeffprior2d(pgammaint, lo, up, Mj, 1);

	Double_t masspoints[maxsig-1];
	Double_t massprior[maxsig-1];

	for(int l=0; l<maxsig-1; l++) {
		cout << sigNames[l] << endl;
		double point = jeffprior2d(sigNames[l]);
		cout << sigNames[l] << ": " << point << endl;
		masspoints[l] = sigNames[l];
		massprior[l] = point;		
	}
